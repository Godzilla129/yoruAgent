...
DIR_KATALOG=/usr/share/yoru/catalog
DIR_ETC=/etc/yoru
...
periksa_lingkungan() {
  ...
  [ -d "$ASAL/katalog" ] || mati "folder katalog tidak ada - jalankan skrip ini dari dalam folder repo"
...
buat_folder() {
  ...
  install -d -o root -g root -m 755 "$DIR_BIN" "$DIR_KATALOG" "$DIR_ETC" "$DIR_LOG" \
...
pasang_katalog() {
  ...
  for f in "$ASAL"/catalog/*.yaml; do
    [ -f "$f" ] || continue
    install -o root -g root -m 644 "$f" "$DIR_KATALOG/" || mati "gagal menyalin $(basename "$f")"
    n=$((n+1))
  done
